...
빌드 도구 없이 CDN 스크립트 태그로 로드. 모든 파일이 전역 스코프를 공유하므로 로드 순서가 의존성을 결정한다.

### 1-1. index.html JS 로드 순서
...
### 1-2. admin.html JS 로드 순서

```
js/admin/gh.js → js/admin/state.js → js/admin/fac.js
→ js/admin/tab-mapping.js → js/admin/tab-calc.js → js/admin/tab-manifest.js
→ js/admin/data-viewer.js → js/admin/cache.js → js/admin/app.js
```

### 1-3. 공유 유틸리티 (js/utils.js)
...
- HTML 구조만 포함 — JS는 `js/` 디렉터리, CSS는 `css/tokens.css` + `css/common.css` + `css/index.css`로 분리
...
- JS는 `js/admin/` 9개 파일로 분리, CSS는 `css/admin.css`로 분리
...
- `left_join`: `true`이면 캐시 생성 시 `sources[1]`(분모 소스)을 primary로 사용 — 분모가 있는 모든 대학을 포함하고 `sources[0]`(분자 소스)에 데이터가 없으면 numerator=null→0 처리. 공시 안 한 대학도 0%로 집계됨. 단일 소스 지표에는 무시됨.
...
### 3-2. manifest.json (`data/manifest.json`)
...
### 3-4. benchmark_cache.json (`data/benchmark_cache.json`)
...
## 4. 캐시 생성 알고리즘 상세 (admin.html `CacheManager`)

### 4-1. 연도 처리 방식 — 지표별 독립 계산

- **지표마다 독립된 `tempMap`** 을 구성해 산식을 계산한 뒤, **지표값(결과)만** `mergedMap`에 저장한다. raw 필드를 지표 간에 공유하지 않는다.
- **왜**: 소스파일에 같은 이름의 필드(예: `재학생_계(C)`)가 있을 때, 다른 지표에서 먼저 다른 연도의 값이 들어오면 올바른 값으로 덮어쓸 수 없는 문제가 있었음.
...
`_buildTempMap`에서 `left_join` 시 분자 소스(sources[0])의 `기준연도→공시연도` 매핑을 사전에 구성(`numSrcYearMap`). 이를 통해 분자 데이터가 없는 기준연도 행을 skip하여 잘못된 0% 집계 방지.

- **⚠ IMPORTANT**: `numSrcYearMap`은 `left_join` 지표의 `_buildTempMap` 진입 시 sources[0] 데이터를 먼저 순회해 `{ 기준연도: 공시연도 }` 맵을 만든다. 이후 primary(sources[1]) 순회 시 해당 `기준연도`가 `numSrcYearMap`에 없으면 그 행은 건너뛴다.
